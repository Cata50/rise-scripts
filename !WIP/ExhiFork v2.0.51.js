...
// MODULE
var module = rise.registerModule("Exhibition", "By Cata50 - v2.0.51 (bed plate size + scaffold block outline + performance fixes)");
...
module.registerSetting("boolean", "Scaffold Block outline", true);
module.registerSetting("boolean", "theme outline", true);
module.registerSetting("color", "outline color", [255, 255, 255]);
module.registerSetting("number", "outline opacity", 100, 26, 255);
module.registerSetting("number", "outline width", 1, 1, 5, 0.1);
...
module.setSettingVisibility("Scaffold Block Outline", false);
module.setSettingVisibility("theme outline", false);
module.setSettingVisibility("outline color", false);
module.setSettingVisibility("outline opacity", false);
module.setSettingVisibility("outline width", false);
...
var scaffoldBlockOutline = module.getSetting("Scaffold Block Outline");
...
module.handle("onTick", function (tick) {
    ...
    var scaffoldBlockOutline = module.getSetting("Scaffold Block Outline");
    var themeOutline = module.getSetting("theme color");
    var outlineColor = module.getSetting("outline color");
    var outlineOpac = module.getSetting("outline opacity");
    var outlineWidth = module.getSetting("outline width");
    ...
    module.setSettingVisibility("Scaffold Block Outline", qol);
    module.setSettingVisibility("theme outline", qol && scaffoldBlockOutline);
    module.setSettingVisibility("outline color", qol && scaffoldBlockOutline);
    module.setSettingVisibility("outline opacity", qol && scaffoldBlockOutline);
    module.setSettingVisibility("outline width", qol && scaffoldBlockOutline);
        ...
        if (extraInfo === "disabled") {
            ...
            if (coloredLetter) { //MAIN
                ...
                if (extraInfo && time) {
                    ...
                    exhibition += sepo + timeString + endc;
                }
                if (extraInfo && fps) {
                    if (extraInfo === "enabled optimized") {
                    ...
                    mcFont.drawWithShadow(sepo, 3 + mcFont.width(exhibition), 4, [185, 185, 185, 255]);
                    mcFont.drawWithShadow(mc.getFPS() + " FPS", 3 + mcFont.width(exhibition + sepo), 4, [255, 255, 255, 255]);
                    mcFont.drawWithShadow(sepc, 3 + mcFont.width(exhibition + sepo + mc.getFPS() + " FPS"), 4, [185, 185, 185, 255]);
                    exhibition += sepo + mc.getFPS() + " FPS" + endc;
                }
                if (extraInfo && ping) {
                    if (extraInfo === "enabled optimized") {
                        sepc = "]"; //editor
                    ...
                    mcFont.drawWithShadow(sepo, 3 + mcFont.width(exhibition), 4, [185, 185, 185, 255]);
                    mcFont.drawWithShadow(rise.getPing() + " ms", 3 + mcFont.width(exhibition + sepo), 4, [255, 255, 255, 255]);
                    mcFont.drawWithShadow(sepc, 3 + mcFont.width(exhibition + sepo + rise.getPing() + " ms"), 4, [185, 185, 185, 255]);
                }
            } else {
                mcFont.drawWithShadow("Exhibition", 3, 3, white);
                if (extraInfo && time) {
...
module.handle("onRender3D", function () {
    //makes functions only work if their module is enabled
    var qol = module.getSetting("QoL");
    var bedEsp = module.getSetting("Bed ESP");
    var scaffoldBlockOutline = module.getSetting("Scaffold Block Outline");
    ...
    if (qol) {
        ...
        if (scaffoldBlockOutline) {
            function renderScaffoldBlockOutline() {
                var themeOutline = module.getSetting("theme outline");
                var outlineColor = module.getSetting("outline color");
                var outlineOpac = module.getSetting("outline opacity");
                var outlineWidth = module.getSetting("outline width");

                if (themeOutline) {
                    outlineColor = render.getThemeColor();
                }

                var outlineR = outlineColor[0] || 255;
                var outlineG = outlineColor[1] || 255;
                var outlineB = outlineColor[2] || 255;
                blockOutlineColor = [outlineR, outlineG, outlineB, outlineOpac];

                var playerPos = player.getPosition();
                var blockPos = world.newBlockPos(Math.floor(playerPos.getX()), Math.floor(playerPos.getY()) - 1, Math.floor(playerPos.getZ()));

                var lines = drawCubeAroundScaffoldBlock(blockPos, 1);
                for (var j = 0; j < lines.length; j++) {
                    var start = lines[j][0];
                    var end = lines[j][1];
                    render.drawLine3D(start, end, blockOutlineColor, outlineWidth);
                }
            }

            function drawCubeAroundScaffoldBlock(blockPos, size) {
                var halfSize = size / 2;
                var x = blockPos.getPosition().getX() + 0.5;
    ...
    if (qol && scaffoldBlockOutline && scaffold.isEnabled()) {
        renderScaffoldBlockOutline();
    }

});


module.handle("onGameEvent", function () {
